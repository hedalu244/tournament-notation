...
function stringifySlash(tokens: Token[]): string {
...
function stringifyBracket(tokens: Token[]): string {
...
function parse(tokens: [Token, ...Token[]]): [Token, ...Token[]] {
  ...
  if (tokens[0].value === "!") return [tokens[0], ...parse(tokens.slice(1))];
  if (tokens[0].value === "]") throw new Error();
  if (tokens[0].value === "[") {
    for(let i = 1, depth = 1; ; i++){
      if (tokens.length <= i) throw new Error();
  ...
  if (tokens[0].path === "/") {
    if (!isWord(tokens[1]) || !tokens[1].path.endsWith("\\")) {
      ...
      if (!isWord(tokens[1]) || !tokens[1].path.endsWith("\\")) throw new Error();
    }
    return parse([{
  ...
  if (tokens[0].path.endsWith("/")) {
      if (!isWord(tokens[1]) || tokens[1].path !== "\\") {
        tokens = [tokens[0], ...parse(tokens.slice(1))];
        if (!isWord(tokens[1]) || tokens[1].path !== "\\") throw new Error();
      }
      return parse([{
