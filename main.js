...
function tokenizeBracket(input) {
    let literals = input.match(/[\\\/]*([^\[\]\!\\\/\s]+)|\[|\]|\!/g);
    if (literals === null)
        literals = [];
    return literals.map(x => {
...
function stringifyIntoSlash(trees) {
...
function stringifyIntoBracket(trees) {
    return trees.map(x => serialize(x)).join("\n");
...
function test() {
    for (var i = 0; i < 10; i++) {
        ...
        if (JSON.stringify(tree) !== JSON.stringify(tree2) || JSON.stringify(tree) !== JSON.stringify(tree3))
            console.log("boom");
        console.log("=====");
    }
}
...
window.onload = () => {
    $("bracket").oninput = () => {
        ...
        try {
            let trees = parseBracket(tokenizeBracket($("bracket").value)).map(x => {
                if (x === "!")
    ...
    $("slash").oninput = () => {
        ...
        $("slash_error").innerText = "";
        try {
            let trees = parseSlash(tokenizeSlash($("slash").value)).map(x => {
                if (x.path !== "")
                    throw new Error("parse error");
                delete x.value.head;
                return x.value;
